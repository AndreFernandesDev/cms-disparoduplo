...
async function queryProcessor(config, doc) {
    ...
    const queries = [];
    let artifactImportIDs = {};
    let storeIdentifiers = {};
    let storeFactories = {};
    let hasVariables = {};
    ...
    await (0, utils_1.walkTaggedDocuments)(config, doc, doc.instance.content, {
        ...
        where(graphqlDoc) {
            return (graphqlDoc.definitions.length === 1 &&
        ...
        onTag(tag) {
            ...
            storeFactories[artifact.name] = AST.identifier((0, common_1.ensureStoreFactoryImport)({
                config,
                body: isRoute ? doc.module.content.body : doc.instance.content.body,
                artifact,
            })[0]);
            storeIdentifiers[artifact.name] = AST.identifier(`store_${storeFactories[artifact.name].name}`);
            artifactImportIDs[artifact.name] = (0, common_1.ensureArtifactImport)({
                config,
                body: isRoute ? doc.module.content.body : doc.instance.content.body,
                artifact: artifact,
            });
            // check if there is a variable function defined
            hasVariables[artifact.name] = Boolean(doc.module.content.body.find((statement) => statement.type === 'ExportNamedDeclaration' &&
            ...
            AST.objectExpression([
                AST.objectProperty(AST.identifier('kind'), AST.stringLiteral(types_1.ArtifactKind.Query)),
                ...
                AST.objectProperty(AST.identifier('artifact'), AST.identifier(artifactImportIDs[artifact.name])),
            ]));
        },
    });
...
function processModule({ config, script, queries, artifactImportIDs, storeIdentifiers, storeFactories, hasVariables, }) {
    ...
    let insertIndex = script.content.body.findIndex((expression) => expression.type !== 'ImportDeclaration');
    for (const query of queries) {
        const name = query.parsedDocument.definitions[0].name
            .value;
        script.content.body.splice(insertIndex, 0, 
        // @ts-expect-error
        AST.variableDeclaration('const', [
            AST.variableDeclarator(storeIdentifiers[name], AST.callExpression(storeFactories[name], [])),
        ]));
    }
...
function addKitLoad({ config, body, queries, artifactImportIDs, storeIdentifiers, hasVariables, }) {
    // look for any hooks
    ...
    const preloadFn = AST.functionDeclaration(AST.identifier('load'), [AST.identifier('context')], 
    // return an object
    AST.blockStatement([
        AST.returnStatement(AST.objectExpression([
            AST.spreadElement(AST.memberExpression(requestContext, AST.identifier('returnValue'))),
            AST.objectProperty(AST.identifier('props'), AST.objectExpression([
                AST.spreadElement(AST.memberExpression(AST.memberExpression(requestContext, AST.identifier('returnValue')), AST.identifier('props'))),
                ...queries.map((query) => {
                    const identifier = AST.identifier(variablesKey(query.parsedDocument.definitions[0]));
    ...
    body.push(AST.exportNamedDeclaration(preloadFn));
    ...
    preloadFn.body.body.splice(insertIndex, 0, AST.variableDeclaration('const', [
        AST.variableDeclarator(requestContext, AST.newExpression(AST.identifier('RequestContext'), [AST.identifier('context')])),
    ]));
    ...
    for (const document of queries) {
        ...
        preloadFn.body.body.splice(nextIndex++, 0, AST.variableDeclaration('const', [
            AST.variableDeclarator(AST.identifier(variableIdentifier), hasVariables[name]
                ? AST.callExpression(AST.memberExpression(requestContext, AST.identifier('computeInput')), [
                    AST.objectExpression([
                        AST.objectProperty(AST.literal('config'), AST.identifier('houdiniConfig')),
                        AST.objectProperty(AST.literal('framework'), AST.stringLiteral(config.framework)),
                        AST.objectProperty(AST.literal('variableFunction'), AST.identifier(queryInputFunction(document.artifact.name))),
                        AST.objectProperty(AST.literal('artifact'), AST.identifier(artifactImportIDs[document.artifact.name])),
                    ]),
                ])
                : AST.objectExpression([])),
        ]));
        if (hasVariables[name]) {
            preloadFn.body.body.splice(nextIndex++, 0, 
            // if we ran into a problem computing the variables
            AST.ifStatement(AST.unaryExpression('!', AST.memberExpression(requestContext, AST.identifier('continue'))), AST.blockStatement([AST.returnStatement(retValue)])));
        }
        const fetchCall = AST.callExpression(AST.memberExpression(storeIdentifiers[document.artifact.name], AST.identifier('fetch')), [
            AST.objectExpression([
                ...
                AST.objectProperty(AST.literal('blocking'), AST.booleanLiteral(!!afterLoadDefinition)),
            ]),
        ]);
        if (needsPromises) {
            ...
            awaitsAndChecks.splice(0, 0, 
            // await the promise
            AST.variableDeclaration('const', [
                AST.variableDeclarator(AST.identifier(preloadKey), AST.awaitExpression(AST.identifier(preloadPromiseKey))),
            ]));
        }
        else {
            preloadFn.body.body.splice(nextIndex++, 0, 
            ...
            AST.variableDeclaration('const', [
                AST.variableDeclarator(AST.identifier(preloadKey), AST.awaitExpression(fetchCall)),
            ]));
        }
    }
    ...
    if (beforeLoadDefinition || afterLoadDefinition || onLoadDefinition) {
        let context = [requestContext, config, queries];
        if (beforeLoadDefinition) {
            preloadFn.body.body.splice(insertIndex, 0, ...loadHookStatements('beforeLoad', ...context));
        }
...
function addSapperPreload(config, body) {
    ...
    const preloadFn = AST.functionDeclaration(AST.identifier('preload'), [AST.identifier('page'), AST.identifier('session')], 
    // all we need to do is invoke the utility
    AST.blockStatement([
        AST.returnStatement(AST.callExpression(AST.identifier('convertKitPayload'), [
            AST.thisExpression(),
            AST.identifier('load'),
            AST.identifier('page'),
...
function loadHookStatements(name, requestContext, config, queries) {
    if (name === 'onLoad') {
        console.warn('Warning: Houdini `onLoad` hook has been renamed to `beforeLoad`. ' +
            'Support for onLoad will be removed in an upcoming release');
    }
    return [
        AST.expressionStatement(AST.awaitExpression(AST.callExpression(AST.memberExpression(requestContext, AST.identifier('invokeLoadHook')), [
            AST.objectExpression([
                AST.objectProperty(AST.literal('variant'), AST.stringLiteral(name === 'afterLoad' ? 'after' : 'before')),
                AST.objectProperty(AST.literal('framework'), AST.stringLiteral(config.framework)),
                AST.objectProperty(AST.literal('hookFn'), AST.identifier(name)),
                // after load: pass query data to the hook
                ...(name === 'afterLoad'
                    ? [
                        AST.objectProperty(AST.literal('input'), afterLoadQueryInput(queries)),
                        AST.objectProperty(AST.literal('data'), afterLoadQueryData(queries)),
                    ]
                    : []),
            ]),
        ]))),
...
function afterLoadQueryInput(queries) {
    return AST.objectExpression(queries.map(({ parsedDocument: { definitions } }) => AST.objectProperty(AST.literal(definitions[0]?.name?.value || null), AST.identifier(variablesKey(definitions[0])))));
}
function afterLoadQueryData(queries) {
    return AST.objectExpression(queries.map(({ parsedDocument: { definitions } }) => AST.objectProperty(AST.literal(definitions[0]?.name?.value || null), AST.memberExpression(AST.identifier(preloadPayloadKey(definitions[0])), AST.identifier('data')))));
}
function processInstance({ config, script, queries, storeIdentifiers, storeFactories, artifactImportIDs, hasVariables, isRoute, }) {
    ...
    let propInsertIndex = script.content.body.findIndex((expression) => expression.type !== 'ImportDeclaration');
    // find all of the props of the component by looking for export let
    const props = script.content.body.filter((statement) => statement.type === 'ExportNamedDeclaration' &&
        statement.declaration?.type === 'VariableDeclaration').flatMap(({ declaration }) => declaration.declarations.map((dec) => dec.id.name));
    ...
    if (!isRoute) {
        for (const query of queries) {
            const operation = query.parsedDocument.definitions[0];
            const name = operation.name.value;
            script.content.body.splice(propInsertIndex++, 0, 
            // @ts-expect-error
            AST.variableDeclaration('const', [
                AST.variableDeclarator(storeIdentifiers[name], AST.callExpression(storeFactories[name], [])),
            ]));
    ...
    // @ts-expect-error
    AST.variableDeclaration('const', [
        AST.variableDeclarator(contextIdentifier, AST.callExpression(AST.identifier('getHoudiniContext'), [])),
    ]));
    ...
    for (const query of queries) {
        ...
        if (isRoute) {
            variableDeclaration = AST.exportNamedDeclaration(AST.variableDeclaration('let', [
        ...
        // if we are processing a non-route, we need to label the variable and compute it in the component body
            ...
            variableDeclaration = AST.labeledStatement(AST.identifier('$'), 
            //
            AST.expressionStatement(AST.assignmentExpression('=', AST.identifier(inputPropName), AST.callExpression(AST.identifier('marshalInputs'), [
                AST.objectExpression([
                    AST.objectProperty(AST.identifier('config'), AST.identifier('houdiniConfig')),
                    AST.objectProperty(AST.identifier('artifact'), AST.identifier(artifactImportIDs[name])),
                    AST.objectProperty(AST.identifier('input'), AST.callExpression(AST.memberExpression(AST.identifier(queryInputFunction(name)), AST.identifier('call')), [
                        contextIdentifier,
                        AST.objectExpression([
                            AST.objectProperty(AST.identifier('props'), 
                            // pass every prop explicitly
                            AST.objectExpression(props.map((prop) => AST.objectProperty(AST.identifier(prop), AST.identifier(prop))))),
                            AST.objectProperty(AST.identifier('session'), AST.memberExpression(contextIdentifier, AST.identifier('session'))),
                        ]),
                    ])),
                ]),
            ]))));
        ...
        script.content.body.splice(propInsertIndex + 1, 0, 
        // @ts-expect-error: recast does not mesh with babel's comment AST. ignore it.
        variableDeclaration, AST.labeledStatement(AST.identifier('$'), AST.expressionStatement(AST.logicalExpression('&&', AST.identifier('isBrowser'), AST.callExpression(AST.memberExpression(storeIdentifiers[name], AST.identifier('fetch')), [
            AST.objectExpression([
                AST.objectProperty(AST.literal('variables'), AST.identifier(inputPropName)),
                AST.objectProperty(AST.literal('context'), contextIdentifier),
            ]),
        ])))));
    }
}
function preloadPayloadKey(operation) {
    return `_${operation.name?.value}`;
}
function variablesKey(operation) {
    return `_${operation.name?.value}_Input`;
}
function queryInputFunction(name) {
    return `${name}Variables`;
}
function findExportedFunction(body, name) {
    return body.find((expression) => expression.type === 'ExportNamedDeclaration' &&
