...
async function typeCheck(config, docs) {
        ...
        graphql.visit(parsed, {
            ...
            [graphql.Kind.DIRECTIVE](directive, _, parent, __, ancestors) {
                ...
                if ((definition.kind !== 'OperationDefinition' &&
                    ...
                    errors.push(new Error(`@${directive.name.value} can only appear in queries or fragments`));
                    return;
                }
                // we need to figure out the type of the list so lets start walking down
                // the list of parents starting at the root type
                let rootType = definition.kind === 'OperationDefinition'
                ...
                for (const parent of parents) {
                    ...
                    if (parent.kind !== 'Field') {
                        errors.push(new types_1.HoudiniErrorTodo("Shouldn't get here"));
                        return;
                    }
                    // if we are looking at a list type
                    if (graphql.isListType(rootType) ||
                ...
                if (directive.name.value === config.paginateDirective) {
                    ...
                    if (needsParent) {
                        errors.push(new types_1.HoudiniErrorTodo(`@${config.paginateDirective} cannot be below a list`));
                    }
                }
                ...
                const nameArg = directive.arguments?.find(({ name }) => name.value === config.listNameArg);
                if (!nameArg) {
                    // if we are looking at @list there is an error
                    if (directive.name.value === config.listDirective) {
                        errors.push(new types_1.HoudiniErrorTodo('Could not find name arg'));
                    }
                    // regardless there's nothing more to process
                    return;
                }
                if (nameArg.value.kind !== 'StringValue') {
                    errors.push(new types_1.HoudiniErrorTodo('Name arg must be a static string, it cannot be set to a variable.'));
                    return;
                }
                // if we have already seen the list name there's a problem
                const listName = nameArg.value.value;
                if (lists.includes(listName)) {
                    errors.push(new types_1.HoudiniErrorTodo('List names must be unique'));
                    return;
                }
                // in order to figure out the targets for the list we need to look at the field
                // definition
                const pType = (0, common_1.parentTypeFromAncestors)(config.schema, filename, ancestors.slice(0, -1));
                const targetField = ancestors[ancestors.length - 1];
                const targetFieldDefinition = pType.getFields()[targetField.name.value];
                const { type, error } = (0, list_1.connectionSelection)(config, targetFieldDefinition, (0, common_1.parentTypeFromAncestors)(config.schema, filename, ancestors), targetField.selectionSet);
                ...
                if (missingIDFields.length > 0) {
                    if (error) {
                        ...
                        errors.push(new types_1.HoudiniErrorTodo(`@${config.listDirective} can only be applied to types with the necessary id fields: ${missingIDFields.join(', ')}.`));
                    }
                    return;
                }
    ...
    const rules = (filepath) => [...graphql.specifiedRules]
        .filter(
...
const validateLists = ({ config, freeLists, lists, listTypes, fragments, }) => function verifyListArtifacts(ctx) {
    return {
        ...
        FragmentSpread(node) {
            ...
            if (!config.isListFragment(node.name.value)) {
                ...
                if (!fragments[node.name.value]) {
                    ctx.reportError(new graphql.GraphQLError('Encountered unknown fragment: ' + node.name.value));
                }
                return;
            }
            ...
            if (!lists.includes(listName)) {
                ctx.reportError(new graphql.GraphQLError('Encountered fragment referencing unknown list: ' + listName));
                return;
            }
            ...
            let directive = node.directives?.find(({ name }) => name.value === config.listParentDirective);
            if (directive) {
            ...
            if (!directive) {
                ctx.reportError(new graphql.GraphQLError('parentID is required for this list fragment'));
                return;
            }
            // find the argument holding the parent ID
            let parentArg = directive.arguments?.find((arg) => arg.name.value === config.listDirectiveParentIDArg);
            if (!parentArg) {
                ctx.reportError(new graphql.GraphQLError('parentID is required for this list fragment'));
                return;
            }
        ...
        Directive(node) {
            ...
            if (directiveName === 'connection') {
                ctx.reportError(new graphql.GraphQLError('@connection was renamed to @list. Please change your components. ' +
            ...
            if (!config.isInternalDirective(node)) {
                ...
                if (!config.schema.getDirective(directiveName)) {
                    ctx.reportError(new graphql.GraphQLError('Encountered unknown directive: ' + directiveName));
                }
                return;
            }
            // if the directive points to a type we don't recognize as the target of a list
            if (config.isListOperationDirective(directiveName) &&
                !listTypes.includes(config.listNameFromDirective(directiveName))) {
                ctx.reportError(new graphql.GraphQLError('Encountered directive referencing unknown list: ' + directiveName));
                return;
            }
        },
    };
            ...
            Directive(node) {
                ...
                for (const arg of node.arguments || []) {
                    ...
                    if (arg.value.kind !== 'ObjectValue') {
                        ctx.reportError(new graphql.GraphQLError('values in @arguments must be an object'));
                        return;
                    }
                    // grab the type argument
                    const typeArg = arg.value.fields.find((field) => field.name.value === 'type');
                    const defaultValue = arg.value.fields.find((field) => field.name.value === 'default');
                    // if there is no type value
                    if (!typeArg) {
                        ctx.reportError(new graphql.GraphQLError('missing type field for @arguments directive'));
                        return;
                    }
                    // make sure that the two types at least match
                    if (typeArg.value.kind !== graphql.Kind.STRING) {
                        ctx.reportError(new graphql.GraphQLError('type field to @arguments must be a string'));
                        return;
                    }
                    ...
                    const defaultValueType = defaultValue.value.kind.substring(0, defaultValue.value.kind.length - 'Value'.length);
                    // if the claimed type does not match the default value there's an error
                    if (typeArg.value.value !== defaultValueType) {
                        ctx.reportError(new graphql.GraphQLError(`Invalid default value provided for ${arg.name.value}. Expected ${typeArg.value.value}, found ${defaultValueType}`));
                        return;
                    }
                }
            },
            FragmentSpread(targetFragment, _, __, ___, ancestors) {
                ...
                if (!requiredArgs[fragmentName]) {
                    ...
                    try {
                        ...
                        args = (0, fragmentVariables_1.fragmentArguments)(config, filepath, fragments[fragmentName]);
                    }
                    catch (e) {
                        ctx.reportError(new graphql.GraphQLError(e.message));
                        return;
                    }
                ...
                const appliedArguments = (0, fragmentVariables_1.withArguments)(config, targetFragment).reduce((map, arg) => ({
                    ...map,
                    [arg.name.value]: arg,
                }), {});
                const appliedArgumentNames = Object.keys(appliedArguments);
                // find the missing arguments
                const missing = requiredArgs[fragmentName].filter((arg) => !appliedArgumentNames.includes(arg));
                if (missing.length > 0) {
                    ctx.reportError(new graphql.GraphQLError('The following arguments are missing from this fragment: ' +
                        JSON.stringify(missing)));
                    return;
                }
                // look for any args that we don't recognize
                const unknown = appliedArgumentNames.filter((arg) => !fragmentArgumentNames[fragmentName].includes(arg));
                if (unknown.length > 0) {
                    ctx.reportError(new graphql.GraphQLError('Encountered unknown arguments: ' + JSON.stringify(unknown)));
                }
                // every argument corresponds to one defined in the fragment
                    ...
                    for (const [applied, target] of zipped) {
                        ...
                        if (applied.value.kind === graphql.Kind.VARIABLE ||
                            applied.value.kind === graphql.Kind.LIST ||
                            applied.value.kind === graphql.Kind.OBJECT) {
                            continue;
                        }
                        // the applied value isn't a variable
                        const appliedType = applied.value.kind.substring(0, applied.value.kind.length - 'Value'.length);
                        // if the two don't match up, its not a valid argument type
                        if (appliedType !== target) {
                            ctx.reportError(new graphql.GraphQLError(`Invalid argument type. Expected ${target}, found ${appliedType}`));
                        }
                    }
                }
            },
    ...
    return function (ctx) {
        ...
        return {
            Directive(node, _, __, ___, ancestors) {
                ...
                if (alreadyPaginated) {
                    ctx.reportError(new graphql.GraphQLError(`@${config.paginateDirective} can only appear in a document once.`));
                }
                // make sure we fail if we see another paginated field
                alreadyPaginated = true;
                // find the definition containing the directive
                const definition = (0, common_1.definitionFromAncestors)(ancestors);
                // look at the fragment arguments
                const definitionArgs = (0, fragmentVariables_1.fragmentArguments)(config, filepath, definition);
                // a fragment marked for pagination can't have requried args
                const hasRequiredArgs = definitionArgs.find((arg) => arg.required);
                if (hasRequiredArgs) {
                    ctx.reportError(new graphql.GraphQLError('@paginate cannot appear on a document with required args'));
                    return;
                }
                // look at the field the directive is applied to
                const targetFieldType = (0, common_1.parentTypeFromAncestors)(config.schema, filepath, ancestors.slice(0, -1));
                ...
                const fieldArgs = type.args.reduce((args, arg) => ({
                    ...args,
                ...
                if (cursorPagination) {
                    ...
                    if (!forward && !backwards) {
                        ctx.reportError(new graphql.GraphQLError('A field with cursor-based pagination must have a first or last argument'));
                    }
                    if (forward && backwards) {
                        ctx.reportError(new graphql.GraphQLError(`A field with cursor pagination cannot go forwards an backwards simultaneously`));
                    }
                    return;
                }
                ...
                if (offsetPagination) {
                    const appliedLimitArg = targetField.arguments?.find((arg) => arg.name.value === 'limit');
                    if (!appliedLimitArg) {
                        ctx.reportError(new graphql.GraphQLError('A field with offset-based pagination must have a limit argument'));
                    }
                    return;
                }
            },
    ...
    return function (ctx) {
        ...
        return {
            ...
            leave: {
                ...
                FragmentDefinition(node) {
                    if (args.size > 0) {
                        ctx.reportError(new graphql.GraphQLError('Encountered unused fragment arguments: ' + [...args].join(',')));
                    }
                },
            },
        };
    ...
    return function (ctx) {
        ...
        return {
            Directive(node, _, __, ___, ancestors) {
                ...
                let definition = (0, common_1.definitionFromAncestors)(ancestors);
                ...
                if (definition.kind === 'OperationDefinition') {
                    ...
                    if (definition.operation !== 'query') {
                        ctx.reportError(new graphql.GraphQLError(`@${node.name.value} must fall on a fragment or query document`));
                        return;
                    }
                ...
                if (!possibleNodes.includes(definitionType)) {
                    ctx.reportError(new graphql.GraphQLError(paginateOnNonNodeMessage(config, node.name.value)));
                }
            },
        };
    };
...
`;
const paginateOnNonNodeMessage = (config, directiveName) => `It looks like you are trying to use @${directiveName} on a document that does not have a valid type resolver.
