...
exports.selectionTypeInfo = exports.inlineType = exports.fragmentKey = void 0;
// externals
...
exports.fragmentKey = '$fragments';
function inlineType({ config, filepath, rootType, selections, root, allowReadonly, body, visitedTypes, missingScalars, }) {
    // start unwrapping non-nulls and lists (we'll wrap it back up before we return)
    const { type, wrappers } = (0, utils_1.unwrapType)(config, rootType);
    let result;
    ...
    // if we are looking at something with a selection set
        ...
        result = AST.tsTypeLiteral([
            ...
            ...(selectedFields || []).filter((field) => field.kind === 'Field').map((selection) => {
                // grab the type info for the selection
                const { type, field } = selectionTypeInfo(config.schema, filepath, rootObj, selection);
                ...
                // we're done
                return (0, types_1.readonlyProperty)(AST.tsPropertySignature(AST.identifier(attributeName), AST.tsTypeAnnotation(attributeType)), allowReadonly);
            }),
        ]);
        ...
        if (fragmentSpreads && fragmentSpreads.length) {
            result.members.push((0, types_1.readonlyProperty)(AST.tsPropertySignature(AST.identifier(exports.fragmentKey), AST.tsTypeAnnotation(AST.tsTypeLiteral((fragmentSpreads || []).map((fragmentSpread) => AST.tsPropertySignature(AST.identifier(fragmentSpread.name.value), AST.tsTypeAnnotation(AST.tsLiteralType(AST.booleanLiteral(true)))))))), allowReadonly));
        }
        ...
        const inlineFragmentSelections = Object.entries(inlineFragments).flatMap(([typeName, fragment]) => {
            ...
            if (objectType.type === 'TSTypeLiteral' &&
                ...
                const existingTypenameIndex = objectType.members.findIndex((member) => member.type === 'TSPropertySignature' &&
                ...
                objectType.members.push((0, types_1.readonlyProperty)(AST.tsPropertySignature(AST.identifier('__typename'), AST.tsTypeAnnotation(AST.tsLiteralType(AST.stringLiteral(typeName)))), allowReadonly));
            }
            ...
            let selectionTypes = Object.entries(inlineFragmentSelections).map(([typeName, { type, tsType }]) => {
                ...
                return AST.tsParenthesizedType(AST.tsIntersectionType([tsType]
                    ...
                    if (type.type === 'TSUnionType') {
                        return type.types.filter((innerType) => innerType.type !== 'TSNullKeyword' &&
    ...
    for (const toWrap of wrappers) {
        if (!root && toWrap === utils_1.TypeWrapper.Nullable) {
            result = (0, types_1.nullableField)(result);
        }
        // if its a non-null we don't need to add anything
        else if (toWrap === utils_1.TypeWrapper.NonNull) {
            continue;
        }
        // it could be a list
...
exports.selectionTypeInfo = selectionTypeInfo;
