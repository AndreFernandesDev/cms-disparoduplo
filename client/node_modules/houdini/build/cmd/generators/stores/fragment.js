...
exports.generateFragmentStore = void 0;
// externals
const path_1 = __importDefault(require("path"));
const utils_1 = require("../../utils");
const pagination_1 = __importDefault(require("./pagination"));
async function generateFragmentStore(config, doc) {
    const storeName = config.storeName(doc);
    const fileName = doc.name;
    const artifactName = `${doc.name}`;
    const paginated = !!doc.refetch?.paginated;
    const paginationExtras = (0, pagination_1.default)(config, doc, 'fragment');
...
${!paginated
        ? ''
        : `import _PaginationArtifact from '${config.artifactImportPath(config.paginationQueryName(doc.name))}'`} 
...
`;
    ...
    await Promise.all([
        ...
        (0, utils_1.writeFile)(path_1.default.join(config.storesDirectory, `${fileName}.js`), storeContent),
    ]);
    // return the store name to the generator so the index file can be created
    return fileName;
}
exports.generateFragmentStore = generateFragmentStore;
