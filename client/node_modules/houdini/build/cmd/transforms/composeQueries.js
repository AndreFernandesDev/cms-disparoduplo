...
async function includeFragmentDefinitions(config, documents) {
    ...
    for (const [index, { name, document, filename }] of documents.entries()) {
        ...
        const operation = document.definitions.find(({ kind }) => kind === graphql_1.Kind.OPERATION_DEFINITION);
        // if there isn't one we don't care about this document
        if (!operation) {
            continue;
        }
        // grab the full list of required fragments
        const allFragments = flattenFragments(filename, { requiredFragments: findRequiredFragments(operation.selectionSet) }, fragments);
...
exports.default = includeFragmentDefinitions;
function collectFragments(config, docs) {
    return docs.reduce((acc, doc) => {
        // look for any definitions in this document
        const definitions = doc.document.definitions.reduce((prev, definition) => definition.kind !== 'FragmentDefinition'
            ? prev
            : {
                ...prev,
