...
const composeQueries_1 = require("./composeQueries");
...
async function fragmentVariables(config, documents) {
    ...
    const fragments = (0, composeQueries_1.collectFragments)(config, documents);
    ...
    for (const doc of documents) {
        ...
        const operation = doc.document.definitions.find(({ kind }) => kind === GraphqlKinds.OPERATION_DEFINITION);
...
function inlineFragmentArgs({ config, filepath, fragmentDefinitions, document, generatedFragments, visitedFragments, scope, newName, }) {
    // if the scope is null, use the root-level scope defined by the document's
    // operation definition
    if (!scope) {
        scope = operationScope(document.definitions.find(({ kind }) => kind === GraphqlKinds.OPERATION_DEFINITION));
    }
    // look up the arguments for the fragment
    const definitionArgs = fragmentArguments(config, filepath, document).reduce((acc, arg) => ({ ...acc, [arg.name]: arg }), {});
        ...
        FragmentSpread(node) {
            ...
            if (!visitedFragments.has(newFragmentName)) {
                ...
                // as look for internal fragment spreads for the referenced fragment
                    ...
                    const definitionIndex = doc.document.definitions.findIndex((definition) => definition.kind === 'FragmentDefinition' &&
...
function withArguments(config, node) {
    const withDirectives = node.directives?.filter((directive) => directive.name.value === config.withDirective);
...
function fragmentArguments(config, filepath, definition) {
    const directives = definition.directives?.filter((directive) => directive.name.value === config.argumentsDirective);
    ...
    directive.arguments?.flatMap((arg) => {
        ...
        let required = false;
        let defaultValue = arg.value.fields?.find((arg) => arg.name.value === 'default')?.value || null;
...
exports.fragmentArguments = fragmentArguments;
function collectDefaultArgumentValues(config, filepath, definition) {
    let result = {};
    for (const { name, required, defaultValue } of fragmentArguments(config, filepath, definition)) {
...
function collectWithArguments(config, filepath, node, scope = {}) {
    ...
    return {
        args,
        hash: '_' + (0, utils_1.murmurHash)(JSON.stringify(args)),
    };
}
exports.collectWithArguments = collectWithArguments;
function operationScope(operation) {
    return (operation.variableDefinitions?.reduce((scope, definition) => ({
        ...scope,
