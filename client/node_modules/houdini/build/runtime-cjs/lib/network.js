...
exports.RequestContext = exports.fetchQuery = exports.convertKitPayload = exports.executeQuery = exports.getCurrentClient = exports.setEnvironment = exports.Environment = exports.HoudiniClient = void 0;
// locals
...
async function fetchQuery({ ${log.yellow('fetch')}, ${log.yellow('session')}, text = '', variables = {} }) {
...
exports.HoudiniClient = HoudiniClient;
...
async function executeQuery({ artifact, variables, session, cached, config, metadata, }) {
...
async function convertKitPayload(context, loader, page, session) {
    // invoke the loader
...
async function fetchQuery({ config, context, artifact, variables, cached = true, policy, }) {
    ...
    if (cached && artifact.kind === 'HoudiniQuery') {
        ...
        if (policy !== types_1.CachePolicy.NetworkOnly) {
            // look up the current value in the cache
            const value = cache_1.default.read({ selection: artifact.selection, variables });
            ...
            else if (policy === types_1.CachePolicy.CacheOnly) {
                return {
                    result: {
                        data: null,
                        errors: [],
    ...
    return {
        result: result.body,
        source: result.ssr ? types_1.DataSource.Ssr : types_1.DataSource.Network,
        partial: false,
    };
}
...
class RequestContext {
    ...
    computeInput({ config, framework, variableFunction, artifact, }) {
        ...
        return (0, scalars_1.marshalInputs)({ artifact, config, input });
    }
}
